...
module.exports = {
    ...
    create: (comentario) => {
        return conn.query('insert into comentario(foto_id, usuario_id, descricao) values($1,$2,$3) returning *', [comentario.foto.id, comentario.usuario.id, comentario.descricao]);
    },
    update: (comentario) => {
        return conn.query('update comentario set foto_id = $1, usuario_id = $2, descricao = $3, status = $4 where id = $5 returning *', [comentario.foto.id, comentario.usuario.id, comentario.descricao, comentario.status, comentario.id]);
    },
    delete: (id) => {
