...
module.exports = {
    ...
    create: async(pessoa) => {
        const pessoaResult = await conn.query('insert into pessoa(nome, email, fone, endereco, cidade_id, usuario_id) values ($1,$2,$3,$4,$5,$6) returning *', [pessoa.nome, pessoa.email, pessoa.fone, pessoa.endereco, pessoa.cidade.id, pessoa.usuario.id]);

        return ajustaAtributos(pessoaResult.rows)[0];
    },

    update: async(pessoa) => {
        const pessoaResult = await conn.query('update pessoa set nome = $1, email = $2, fone = $3, endereco = $4, status = $5, cidade_id = $6, usuario_id = $7 where id = $8 returning *', [pessoa.nome, pessoa.email, pessoa.fone, pessoa.endereco, pessoa.status, pessoa.cidade.id, pessoa.usuario.id, pessoa.id]);
