...
module.exports = {
    ...
    create: async(tabelapreco) => {
        const tabelaprecoResult = await conn.query('insert into tabelapreco(nome, fator) values($1,$2) returning *', [tabelapreco.nome, tabelapreco.fator]);
        return tabelaprecoResult.rows[0];
    },
    update: async(tabelapreco) => {
        const tabelaprecoResult = await conn.query('update tabelapreco set nome = $1, fator = $2 where id = $3 returning *', [tabelapreco.nome, tabelapreco.fator, tabelapreco.id]);
        return tabelaprecoResult.rows[0];
    },
    delete: async(id) => {
